...
Prebuilt `libglfw.so` (x86_64 Linux) is available in [Releases](https://github.com/jdkeke142/glfw-wayland-minecraft/releases). Built on Ubuntu 24.04 via CI, should work on any recent glibc-based distro.

Download it, put it somewhere permanent, and add these JVM arguments in your launcher:
```
...
Note: `-Dorg.lwjgl.librarypath` does NOT work. LWJGL extracts its bundled `.so` from the jar first and ignores the library path. `glfw.libname` forces a specific file.
...
Only needed if you want to modify patches or build for a different architecture.
